...
The client receives the server's response by reading from the input stream using `input.read(response)`. The response is then displayed to the user. If the response starts with the character 'C', indicating that the client guessed the correct number, the `gameOver` flag is set to true, and the loop is
